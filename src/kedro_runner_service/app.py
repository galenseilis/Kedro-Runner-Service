...
def run_kedro_project(request: KedroRunRequest, sender_ip: str):
    ...
    if not project_config:
        raise HTTPException(status_code=404, detail="Project not found in the configuration")
    ...
    request_id = None
    if db_engine:
        request_id = record_request_in_db(request.project, request.params, "queued", sender_ip)
...
@app.post("/run_kedro")
def add_to_queue(request: KedroRunRequest, background_tasks: BackgroundTasks, client_request: Request):
